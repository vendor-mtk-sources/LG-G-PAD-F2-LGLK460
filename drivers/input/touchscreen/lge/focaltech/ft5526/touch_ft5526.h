...
#ifndef LGE_TOUCH_FT5526_H
#define LGE_TOUCH_FT5526_H

#define FT5526_ESD_SKIP_WHILE_TOUCH_ON
/* focaltech APK debug */
...
struct ft5526_touch_info {
	...
	u32 button_cnt:3;
	u32 palm_bit:16;
	struct ft5526_touch_data data[10];
	/* debug info */
	struct ft5526_touch_debug debug;
} __packed;
...
#if 0
struct ft5526_version {
	u8 build : 4;
	u8 major : 4;
	u8 minor;
};

struct ft5526_ic_info {
	struct ft5526_version version;
	u8 product_id[8];
	u8 image_version[2];
...
struct ft5526_data {
	struct device *dev;
	struct kobject kobj;
	struct ft5526_touch_info info;
	struct ft5526_ic_info ic_info;
//	struct ft5526_asc_info asc;	/* ASC */
...
#ifdef CONFIG_LGD_FT5526_FHD_VIDEO_LCD_PANEL
...
static inline struct ft5526_data *to_ft5526_data(struct device *dev)
{
	return (struct ft5526_data *)touch_get_device(to_touch_core(dev));
}

static inline struct ft5526_data *to_ft5526_data_from_kobj(struct kobject *kobj)
{
	return (struct ft5526_data *)container_of(kobj,
			struct ft5526_data, kobj);
}
...
int ft5526_tc_driving(struct device *dev, int mode);
...
static inline int ft5526_read_value(struct device *dev,
					u16 addr, u32 *value)
{
	return ft5526_reg_read(dev, addr, value, sizeof(*value));
}

static inline int ft5526_write_value(struct device *dev,
					 u16 addr, u32 value)
{
	return ft5526_reg_write(dev, addr, &value, sizeof(value));
}
