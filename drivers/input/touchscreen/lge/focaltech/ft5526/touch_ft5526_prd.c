...
#define RAW_DATA_MIN 9000
#define RAW_DATA_MARGIN 0
#define IB_MAX 65
#define IB_MIN 3
#define NOISE_MAX 240
#define NOISE_MIN 0
#define LPWG_RAW_DATA_MAX 12000
#define LPWG_RAW_DATA_MIN 9000
#define LPWG_IB_MAX 65
#define LPWG_IB_MIN 3
#define LPWG_NOISE_MAX 240
...
#define LOG_BUF_SIZE		(16*1024)// 4*1024
...
int min_data = 0;
u8 ic_type = 0x00;
int rawdata_fir = 1; // 0 : off , 1: on => default is on.
...
static int sdcard_spec_file_read(struct device *dev)
{
	...
	int ret = 0;
	int fd = 0;
	char *path[3] = { "/mnt/sdcard/th8_limit.txt",
		"/data/touch/th8_limit.txt",
		"/data/touch/th8_limit_mfts.txt"
	};
	int path_idx = 0;
...
static int spec_get_limit(struct device *dev, char *breakpoint, u16 (*buf)[MAX_COL])
{
	int p = 0;
	int q = 0;
	int r = 0;
	...
	memset(buf, 0, MAX_ROW * MAX_COL * 2);
...
int ft5526_change_op_mode(struct device *dev, u8 op_mode)
{
	...
	data = 0x00;
	ret = ft5526_reg_read(dev, 0x00, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	data = op_mode;
	ret = ft5526_reg_write(dev, 0x00, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
#if 0 // Change report Rate for LV3 only.
	TOUCH_I("d->state = %d\n", d->state);
	if((op_mode == FTS_FACTORY_MODE) && (d->state == TC_STATE_LPWG)) {
		TOUCH_I("[0x88] = 0x01 write\n");
		data = 0x01;
		ret = ft5526_reg_write(dev, 0x88, &data, 1);
		if(ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
...
int ft5526_switch_cal(struct device *dev, u8 cal_en)
{
#if 0
	int i = 0;
	u8 ret;
	...
	data = 0x00;
	ret = ft5526_reg_read(dev, 0xEE, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	data = cal_en;
	ret = ft5526_reg_write(dev, 0xEE, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
int ft5526_prd_check_ch_num(struct device *dev)
{
	u8 ret;
	u8 data;
	...
	ret = ft5526_reg_read(dev, 0x02, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	ret = ft5526_reg_read(dev, 0x03, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
int ft5526_prd_get_raw_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
	...
	data = 0x00;
	ret = ft5526_reg_write(dev, 0x06, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	for (k = 0; k < 3; k++)
	{
		...
		if(bscan==true){
			/* Read Raw data */
			rawdata = 0xAA;    // FT5526 RAWDATA ADDRESS
			ret = ft5526_reg_write(dev, 0x01, &rawdata, 1);
			if(ret < 0) {
				TOUCH_E("i2c error\n");
				return ret;
			}
			...
			ret = ft5526_reg_read(dev, 0x36, &i2c_data[0], MAX_COL*MAX_ROW*2);    // TH8 Column:34 Row:21
			if(ret < 0) {
				TOUCH_E("i2c error\n");
				return ret;
			}
	...
	// Data Select to raw data
	data = 0x00;
	ret = ft5526_reg_write(dev, 0x06, &data, 1);  // 0: RAWDATA
	if(ret < 0) {
...
int ft5526_prd_get_raw_data_sd(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
	...
	ret = ft5526_reg_write(dev, 0x0A, &data, 1);
	if (ret < 0) {
	...
	ret = ft5526_reg_write(dev, 0xFB, &data, 1);
	if (ret < 0) {
	...
	for (k = 0; k < 3; k++)
	{
		...
		if(bscan==true) {
			...
			ret = ft5526_reg_read(dev, 0x36, &i2c_data[0], MAX_COL*MAX_ROW*2);
			if(ret < 0) {
...
int ft5526_prd_get_scap_raw_data(struct device *dev)
{
	int i, k;
	int ret = 0;
	u8 data = 0x00;
	...
	scap_rawdata = 0xAC;
	ret = ft5526_reg_write(dev, 0x01, &scap_rawdata, 1);
	if(ret < 0) {
	...
	ret = ft5526_reg_read(dev, 0x36, &i2c_data[0], SCAP_BUF_SIZE * 2);
	if(ret < 0) {
...
int ft5526_prd_get_cb_data(struct device *dev)
{
	int i, k;
	int ret = 0;
	u8 data = 0x00;
	...
	for (k = 0; k < 3; k++)
	{
		TOUCH_I("Start first SCAN (%d/3)\n", k+1);
		ret = ft5526_reg_read(dev, 0x00, &data, 1);
		if (ret < 0) {
			TOUCH_E("start first scan read error\n");
			return ret;
		}
		data |= 0x80; // 0x40|0x80
		ret = ft5526_reg_write(dev, 0x00, &data, 1);
		...
		for (i = 0; i < 5; i++) {
			...
			if (data == 0x40) {
				TOUCH_I("First SCAN Success : 0x%X, %d ms \n", data, i*20);
				break;
			}
			mdelay(20);
		}
		...
		TOUCH_E("First SCAN Fail (%d/3)\n", k+1);
	}

	if (k >= 3) {
		TOUCH_E("First SCAN failed\n");
		return -EPERM;
	}
#endif

	cb_data = 0x01;
	ret = ft5526_reg_write(dev, 0x44, &cb_data, 1);
	if(ret < 0) {
	...
	/* Read CB data */
	cb_data = 0x00;
	ret = ft5526_reg_write(dev, 0x45, &cb_data, 1);
	if(ret < 0) {
	...
	ret = ft5526_reg_read(dev, 0x4E, &i2c_data[0], SCAP_BUF_SIZE);
	if(ret < 0) {
...
int ft5526_prd_get_noise_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
//	u8 frame_count;
	...
	//low  byte
	data = 0x32;
	ret = ft5526_reg_write(dev, 0x12, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	//high  byte
	data = 0x00;
	ret = ft5526_reg_write(dev, 0x13, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	// Start Noise Test
	data = 0x01;
	ret = ft5526_reg_write(dev, 0x11, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	// Clear Raw Data Addr
	data = 0xAD;
	ret = ft5526_reg_write(dev, 0x01, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	for (i = 0; i < 100; i++)
	{
		ret = ft5526_reg_read(dev, 0x11, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
	...
	ret = ft5526_reg_read(dev, 0x8d, &i2c_data[0], MAX_COL*MAX_ROW*2);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
int ft5526_prd_get_ib_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
	...
	for (i = 0; (total - TEST_PACKET_LENGTH*i) > 0; i++)
	{
		...
		ret = ft5526_reg_write(dev, 0x18, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
		...
		data = (offset & 0x00FF);
		ret = ft5526_reg_write(dev, 0x19, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
		...
		ret = ft5526_reg_read(dev, 0x6E, &i2c_data[offset], read_len);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
...
int ft5526_prd_get_delta_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
	...
	/* Read Raw data */
	data = 0xAA;    // FT5526 RAWDATA ADDRESS
...
int ft5526_prd_get_adc_data(struct device *dev)
{
	int i;
	int ret = 0;
	u8 data = 0x00;
	...
	data |= 0x01; // 0x00|0x01
	ret = ft5526_reg_write(dev, 0x07, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	for (i = 0; i < 5; i++) {
		ret = ft5526_reg_read(dev, 0x07, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
	...
	if(bscan==true){
		/* Read ADC data */
		ret = ft5526_reg_read(dev, 0xF4, &i2c_data[0], ADC_BUF_SIZE*2);	  // TH8 Column:34 Row:21
...
int ft5526_prd_test_data_adc(struct device *dev, int* test_result)
{
	struct ft5526_data *d = to_ft5526_data(dev);
	...
	*test_result = TEST_PASS;
	...
	ret += snprintf(log_buf + ret, LOG_BUF_SIZE - ret, "< ADC : Channel to Ground >\n");
	...
	ret += snprintf(log_buf + ret, LOG_BUF_SIZE - ret, "============= ADC Open Short Test Result : Channel to Ground =============\n");

	for (i=0; i< MAX_ROW + MAX_COL; i++){
		fGShortResistance[i] = -1;
	}
	...
	for (i=0; i< MAX_ROW + MAX_COL; i++){
		...
		else
		{
			if ( iGDrefn - iDsen <= 0 )
			{
				fGShortResistance[i] = 1200;    // WeakShortTest_CC = 1200
				continue;
			}
			...
			fGShortResistance[i] = (((iDsen - iDoffset + 384) * 10 / (iGDrefn - iDsen) * 57) - 12) / 10;
		}

		if( fGShortResistance[i] < 0 )
			fGShortResistance[i] = 0;

		if( ( fGShortResistance[i] < 1200) || ( iDsen - iDoffset < 0 ) ) {
			++fail_count;
	...
	for (i = 0; i < MAX_ROW + MAX_COL; i++) {
		if (fGShortResistance[i] == -1 )
			ret += snprintf(log_buf + ret, LOG_BUF_SIZE - ret, "[%2d] %6d : OK\n", i+1, fGShortResistance[i]);
		else
	...
	ret += snprintf(log_buf + ret, LOG_BUF_SIZE - ret, "============= ADC Open Short Test Result : Channel to Channel=============\n");

	fail_count = 0;

	for(i = 0; i < (MAX_ROW + MAX_COL); ++i)
	{
		fMShortResistance[i] = -1;
	}
	...
	for(i = 0; i < MAX_ROW + MAX_COL; i++)
	{
		const int iDsen = fts_data_adc_buff[i + MAX_ROW + MAX_COL + 3]; // 1 + 1 + TX_NUM + RX_NUM + 1 + (TX_NUM+RX_NUM)
		if(ic_type <= 0x05 || ic_type == 0xff)
		{
			if(iDsen - iMDrefn < 0)  {
				continue;
		...
		else
		{
			if ( iMDrefn - iDsen <= 0 )
			{
				fMShortResistance[i] = 1200;  // WeakShortTest_CC 1200
				continue;
			}
		...
		if( fMShortResistance[i] <= 0  || fMShortResistance[i] < 1200 )
		{
			++fail_count;
			*test_result = TEST_FAIL;
		}
...
int ft5526_prd_test_data(struct device *dev, int test_type, int* test_result)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	struct ft5526_data *d = to_ft5526_data(dev);
	...
	*test_result = TEST_FAIL;
	...
	switch (test_type) {
		case RAW_DATA_TEST:
			...
			spec_get_limit(dev, "Rawdata_Lower", LowerImage);
			spec_get_limit(dev, "Rawdata_Upper", UpperImage);
			break;
		case IB_DATA_TEST:
...
int ft5526_prd_scap_test_data(struct device *dev, int test_type, int* test_result)
{
	struct ft5526_data *d = to_ft5526_data(dev);
	...
	*test_result = TEST_FAIL;
	...
	switch (test_type) {
		case SCAP_RAW_DATA_TEST:
			...
			break;
		case CB_TEST:
			ret += snprintf(log_buf + ret, LOG_BUF_SIZE - ret, "============= CB Test Result =============\n");
			limit_upper = CB_MAX;
			limit_lower = CB_MIN;
			break;
		default:
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	int noise_test_result = TEST_FAIL;
	int open_short_test_result = TEST_FAIL;
	...
	ret = ft5526_reg_read(dev, 0xB1, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	// Change clb switch
	ret = ft5526_switch_cal(dev, 1);
	if(ret < 0)
		goto EXIT;

	// Change to factory mode
	ret = ft5526_change_op_mode(dev, FTS_FACTORY_MODE);
	if(ret < 0)
		goto EXIT;
	...
	ret = ft5526_prd_check_ch_num(dev);
	if(ret < 0)
		goto EXIT;

#if 1
	ret = ft5526_prd_get_raw_data_sd(dev);
#else
	ret = ft5526_prd_get_raw_data(dev);
#endif

	if(ret < 0)
		goto EXIT;

	ret_size = ft5526_prd_test_data(dev, RAW_DATA_TEST, &rawdata_test_result);
	...
	ret = ft5526_prd_get_ib_data(dev);
	if(ret < 0)
		goto EXIT;

	ret_size = ft5526_prd_test_data(dev, IB_DATA_TEST, &test_result);
	...
	ret = ft5526_prd_get_noise_data(dev);
	if(ret < 0)
		goto EXIT;

	ret_size = ft5526_prd_test_data(dev, NOISE_TEST, &noise_test_result);
	...
	ret = ft5526_prd_get_scap_raw_data(dev);
	if(ret < 0)
		goto EXIT;

	ret_size = ft5526_prd_scap_test_data(dev, SCAP_RAW_DATA_TEST, &scap_rawdata_test_result);
	...
	ret = ft5526_prd_get_cb_data(dev);
	if(ret < 0)
		goto EXIT;

	ret_size = ft5526_prd_scap_test_data(dev, CB_TEST, &cb_test_result);
	...
	/* ADC open_short test */
	TOUCH_I("Show_sd : Open Short test\n");

	memset(log_buf, 0, LOG_BUF_SIZE);

	ret = ft5526_prd_get_adc_data(dev);
	if(ret < 0)
		goto EXIT;

	ret_size = ft5526_prd_test_data_adc(dev, &open_short_test_result);

	TOUCH_I("Open Short Test Result : %d\n", open_short_test_result);
...
EXIT:
	...
	ret = ft5526_change_op_mode(dev, FTS_WORK_MODE);
	if(ret < 0)
		TOUCH_I("Failed to return WORK_MODE\n");

	if (noise_test_result && open_short_test_result && scap_rawdata_test_result && cb_test_result)
	...
	TOUCH_I("Channel Status (noise_test) : %s", (noise_test_result == TEST_PASS) ? "Pass\n" : "Fail\n");
	TOUCH_I("Channel Status (open_short_test) : %s", (open_short_test_result == TEST_PASS) ? "Pass\n" : "Fail\n");
	TOUCH_I("Channel Status (Scap Raw Data Test) : %s", (scap_rawdata_test_result == TEST_PASS) ? "Pass\n" : "Fail\n");
	TOUCH_I("Channel Status (CB Test) : %s", (cb_test_result == TEST_PASS) ? "Pass\n" : "Fail\n");
...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata)
{
	...
	ft5526_reg_write(dev, rawdata_ctl_write, (u8 *)&wdata, sizeof(wdata));
	TOUCH_I("wdata = %d\n", wdata);
	...
	do {
		...
		if (retry != 1000)
			touch_msleep(5);
		ft5526_reg_read(dev, rawdata_ctl_read,
	...
	ft5526_reg_write(dev, tc_tsp_test_data_offset, (u8 *)&data_offset, sizeof(u32));

	ft5526_reg_read(dev, tc_tsp_data_access_addr, (u8 *)buf, __frame_size);
...
//	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	...
	ret = ft5526_prd_get_delta_data(dev);
	if(ret < 0)
		goto FAIL;
	...
	ret_size = ft5526_prd_test_data(dev, DELTA_SHOW, &test_result);
...
static ssize_t show_rawdata(struct device *dev, char *buf)
{
	...
	//ret_size = ft5526_prd_test_data(dev, RAW_DATA_TEST, &test_result);
	memcpy(buf, log_buf, ret_size);
	TOUCH_I("Raw Data Test Result : %d\n", test_result);
	//printk("%s\n", log_buf);
	touch_interrupt_control(ts->dev, INTERRUPT_ENABLE);
...
static ssize_t show_scap_rawdata(struct device *dev, char *buf)
{
	...
	ret_size = ft5526_prd_scap_test_data(dev, SCAP_RAW_DATA_TEST, &test_result);
	memcpy(buf, log_buf, ret_size);
	TOUCH_I("SCAP Raw Data Test Result : %d\n", test_result);
...
static ssize_t show_cb(struct device *dev, char *buf)
{
	...
	ret_size = ft5526_prd_scap_test_data(dev, CB_TEST, &test_result);
	memcpy(buf, log_buf, ret_size);
...
//	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	...
	ret_size = ft5526_prd_test_data(dev, NOISE_TEST, &test_result);
	memcpy(buf, log_buf, ret_size);
	TOUCH_I("Noise Test Result : %d\n", test_result);
	//printk("%s\n", log_buf);
	mutex_unlock(&ts->lock);
...
static ssize_t show_adc(struct device *dev, char *buf) {
	...
	TOUCH_I("Show Adc Data OK !!!\n");

	ret_size = ft5526_prd_test_data_adc(dev, &test_result);

	memcpy(buf, log_buf, ret_size);
	TOUCH_I("Show Adc Data Result : %d\n", test_result);
...
//	int open_short_test_result = TEST_FAIL;
	...
	ret_size = ft5526_prd_test_data(dev, LPWG_RAW_DATA_TEST, &rawdata_test_result);
	...
	ret_size = ft5526_prd_test_data(dev, LPWG_IB_DATA_TEST, &test_result);
	...
	ret_size = ft5526_prd_test_data(dev, LPWG_NOISE_TEST, &noise_test_result);
...
EXIT:
	...
	// Test result
	ret = snprintf(log_buf, LOG_BUF_SIZE, "\n========RESULT=======\n");
	ret += snprintf(log_buf + ret, LOG_BUF_SIZE - ret, "LPWG RawData : %s", (rawdata_test_result == TEST_PASS)? "Pass\n" : "Fail\n");
	...
	TOUCH_I(" \n========RESULT=======\n");
	TOUCH_I("LPWG RawData : %s", (rawdata_test_result == TEST_PASS)? "Pass\n" : "Fail\n");
...
static ssize_t show_rawdata_fir_on(struct device *dev, char *buf)
{
	rawdata_fir = 1;
	return 0;
}

static ssize_t show_rawdata_fir_off(struct device *dev, char *buf)
{
	rawdata_fir = 0;
	return 0;
}
...
static TOUCH_ATTR(adc_test, show_adc, NULL);
static TOUCH_ATTR(scap_rawdata, show_scap_rawdata, NULL);
static TOUCH_ATTR(cb, show_cb, NULL);
static TOUCH_ATTR(rawdata_fir_on, show_rawdata_fir_on, NULL);
static TOUCH_ATTR(rawdata_fir_off, show_rawdata_fir_off, NULL);
...
//	&touch_attr_ib_test.attr,
	...
	&touch_attr_adc_test.attr,
	&touch_attr_scap_rawdata.attr,
	&touch_attr_cb.attr,
	&touch_attr_rawdata_fir_on.attr,
	&touch_attr_rawdata_fir_off.attr,
	NULL,
};
	...
	char *buf, loff_t off, size_t count)
{
	...
	struct ft5526_data *d = to_ft5526_data(ts->dev);
	...
	if (off == 0) {
		...
		ret = ft5526_switch_cal(ts->dev, 0);

		ret = ft5526_change_op_mode(ts->dev, FTS_FACTORY_MODE);

		if (rawdata_fir == 1)
			ret = ft5526_prd_get_raw_data(ts->dev);
		else
			ret = ft5526_prd_get_raw_data_sd(ts->dev);

		// Data Select to raw data
		data = 0x01;
		ret = ft5526_reg_write(ts->dev, 0x06, &data, 1);

		ret = ft5526_change_op_mode(ts->dev, FTS_WORK_MODE);

		TOUCH_I("Show Raw Data OK !!!\n");

		ret_size = ft5526_prd_test_data(ts->dev, RAW_DATA_TEST, &test_result);
...
static int ft5526_prd_attr_rawdata_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft5526_data *d = to_ft5526_data(dev);
...
int ft5526_prd_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	//ft5526_prd_attr_delta_init(dev);
	ft5526_prd_attr_rawdata_init(dev);
