...
#include "../../../misc/mediatek/video/mt6755/disp_recovery.h"
...
#if defined(TARGET_MT6735_K6P) || defined(CONFIG_LGE_TOUCH_LG4894)
	ts->driver->power(ts->dev, POWER_HW_RESET);
#else
	ts->driver->power(ts->dev, POWER_OFF);
	ts->driver->power(ts->dev, POWER_ON);
...
#if defined(CONFIG_LGE_TOUCH_MELFAS_MIT300)
