...
unsigned int temp_thre=0;
static u8 thre_flag = 0;
...
static int thermal_value=0;
static ktime_t ktime;
static ktime_t ktime_cal;
/*! struct sx9310
...
static ssize_t onoff_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
 ...
 static ssize_t regproxdata_show(struct device *dev, struct device_attribute *attr, char *buf)
 {
	...
	SAR_INFO("Register stat SAR Sensor 1\n");
		...
		for (counter = 0; counter < numberOfButtons; counter++) {
			...
			case IDLE:	/* Button is not being touched! */
				if (((i & pCurrentButton->mask) == pCurrentButton->mask)) {
					...
					hrtimer_start( &this->hr_timer, ktime, HRTIMER_MODE_REL );
			...
			case ACTIVE:	/* Button is being touched! */
				if (((i & pCurrentButton->mask) != pCurrentButton->mask)) {
					...
					channel0_thre = temp_thre;
					thre_flag = 0;
					write_register(this, SX9310_CPS_CTRL9_REG, channel0_thre);
...
static void input_poll_work_func(struct work_struct *work)
{
	...
	u8 data = 0;
	thermal_value = read_thermal("sys/class/thermal/thermal_zone0/temp");
	thermal_value = thermal_value/1000;
	stat = container_of((struct work_struct *) work,
			struct sx9310_data, input_poll_work);
	read_register(stat, 0x35, &data);
	SAR_INFO("state %d, data %d, thre %d, thermal %d SAR Controller 1\n",psmtcButtons[0].state,data,thre_flag,thermal_value);
	if( psmtcButtons[0].state == ACTIVE && data > 4 && thre_flag == 0 && ( thermal_value > 50 || thermal_value < 0) ) {
		thre_flag = 1;
		temp_thre = channel0_thre;
		channel0_thre = 0xD1;
...
enum hrtimer_restart check_threshold( struct hrtimer *timer )
{
  SAR_INFO("watching SAR Controller 1()\n");

  queue_work(psx9310_data->hr_timer_poll_work_queue, &psx9310_data->input_poll_work);
  hrtimer_start( &psx9310_data->hr_timer, ktime_cal, HRTIMER_MODE_REL );
...
reset_proc:
	...
	input->name = "lge_sar_wifi";
	input->id.bustype = BUS_I2C;
	input->dev.init_name = "lge_sar_wifi";
	...
	psx9310_data->hr_timer_poll_work_queue = create_workqueue("hr_timer_poll_wq");
	hrtimer_init( &psx9310_data->hr_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL );
	ktime = ktime_set( 300, 0 );
	ktime_cal = ktime_set( 120, 0 );
	psx9310_data->hr_timer.function = &check_threshold;
