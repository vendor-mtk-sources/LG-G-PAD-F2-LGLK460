...
extern bool lge_get_laf_mid(void);
...
static int lge_atd_mid_poweroff_thread(void *data){
	...
	if( lge_atd_status() &&
		...
		while(loopcnt-- > 0) {
			pr_err("waiting for ATD power off thread, loopcnt : %d , ATD_STATUS : %d, CABLE_CON :%d \n",
									loopcnt,lge_atd_status(),usb_cable_connected());
			msleep(100);
		}
#ifdef USER_BUILD
		kernel_power_off();
#else
		panic("timeout for ATD power off thread");
#endif
	}
