...
CHR_CURRENT_ENUM g_temp_parallel_CC_value = CHARGE_CURRENT_0_00_MA;
...
static unsigned int charging_full_check(void)
{
	...
	/* check EOC if cv is high enough */
	if (g_cv_voltage < cv_voltage)
		return KAL_FALSE;
...
static bool mtk_is_pep_series_connect(void)
{
	if (mtk_pep20_get_is_connect() || mtk_pep_get_is_connect()) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] pep20_is_connect = 1 \n", __func__);
		return true;
	}
	battery_log(BAT_LOG_CRTI, "%s: [PE+] pep20_is_connect = 0 \n", __func__);
	return false;
}

static bool mtk_is_pep_series_operational(void)
{
	if (lge_is_factory_cable_boot()) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] Skip : factory_cable_boot\n", __func__);
		return false;
	}
	/* input current suspended */
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] Skip : CHR_ERROR\n", __func__);
		return false;
	}
	/* battery charging suspended */
	if (BMT_status.bat_charging_state == CHR_HOLD) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] Skip : CHR_HOLD\n", __func__);
		return false;
	}
	/* battery raw soc 100 by HW Request */
	if (BMT_status.SOC >= 100) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] Skip : Raw SOC over 100\n", __func__);
		mtk_pep20_set_is_enable(false);
		mtk_pep20_set_is_connect(false);
		return false;
	}
	/* battery full */
	/* Block by HW Request */
	/*if (BMT_status.bat_charging_state == CHR_BATFULL) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] Skip : CHR_BATFULL\n", __func__);
		return false;
	}*/
	/* pep need to draw more that 300mA from charger */
	if (g_temp_input_CC_value <= CHARGE_CURRENT_300_00_MA) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] Skip : IUSB under 300mA\n", __func__);
		return false;
	}
	/* battery charging is too low */
	if (g_temp_CC_value <= CHARGE_CURRENT_300_00_MA) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] Skip : IBAT under 300mA\n", __func__);
		return false;
	}
	battery_log(BAT_LOG_CRTI, "%s: [PE+] Operation\n", __func__);
	return true;
}

static bool mtk_is_pep_action(void)
{
	if (lge_is_factory_cable_boot()) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] Skip : factory_cable_boot\n", __func__);
		return false;
	...
	battery_log(BAT_LOG_CRTI, "%s: [PE+] Action\n", __func__);
	return true;
}
...
static void select_charging_current(void)
{
	...
	/* make sure input current do not exceed maximum input current */
	...
	battery_log(BAT_LOG_FULL, "%s: set Ichg=%dmA Iinlim=%dmA\n", __func__, g_temp_CC_value / 100, g_temp_input_CC_value / 100);
...
static void select_charging_current_parallel(void)
{
	if (!g_temp_input_CC_value) {
		g_temp_parallel_CC_value = 0;
	} else if (g_temp_CC_value > CHARGE_CURRENT_1800_00_MA) {
		g_temp_parallel_CC_value = g_temp_CC_value / 2;
		g_temp_CC_value = g_temp_parallel_CC_value;
	} else {
		g_temp_parallel_CC_value = 0;
	}

	battery_log(BAT_LOG_CRTI, "[BATTERY]%s: Set Total IBAT=%dmA Parallel IBAT=%dmA\n",
			__func__, g_temp_CC_value / 100, g_temp_parallel_CC_value / 100);
}
...
static void mtk_select_ichg_aicr(void)
{
	...
	if (get_usb_current_unlimited()) {
		...
		if (g_parallel_flag)
			select_charging_current_parallel(); // Set Parallel Charging Current
		battery_log(BAT_LOG_FULL,
			"[BATTERY] %s: select_charging_curret !\n", __func__);
	}
	...
	if (g_parallel_flag)
		battery_charging_control(CHARGING_CMD_SET_CURRENT_SLAVE,
				&g_temp_parallel_CC_value);

	battery_log(BAT_LOG_CRTI,
		"[BATTERY] CC mode input current : %dmA, charging : %dmA, parallel charging : %dmA\n",
		g_temp_input_CC_value / 100, g_temp_CC_value / 100, g_temp_parallel_CC_value / 100);

	BMT_status.input_current_max = g_temp_input_CC_value;
	BMT_status.constant_charge_current_max = g_temp_CC_value + g_temp_parallel_CC_value;

	/* check possible to draw more than 300mA from charger to enable pep */
	if (!mtk_is_pep_series_operational()) {
...
static void pchr_turn_on_charging(void)
{
	...
	battery_log(BAT_LOG_FULL, "[BATTERY] pchr_turn_on_charging(), enable=%d,%d\n",
			power_path_enable, charging_enable);
}
...
PMU_STATUS BAT_BatteryStatusFailAction(void)
{
	...
	if (charging_enabled_check()) {
		battery_log(BAT_LOG_CRTI, "[BATTERY] charging enabled\n");
...
void mt_battery_charging_algorithm(void)
{
	...
	if (lge_is_factory_cable())
		goto charging_algorithm;
#endif

	/* do not run aicr when usb inserted */
	if (BMT_status.charger_type == STANDARD_HOST)
		goto charging_algorithm;
	...
	if (mtk_is_pep_series_connect() && BMT_status.bat_charging_state == CHR_PRE) {
		battery_log(BAT_LOG_CRTI, "%s: [PE+] End of detection, Fastchg update\n", __func__);
		mt_battery_update_status();
	}

charging_algorithm:
	/* check charging charging enabled */
	if (!charging_enabled_check())
		BMT_status.bat_charging_state = CHR_ERROR;
