...
#define CONFIG_LGE_PM_CHARGE_SCENARIO_OTP
#define CONFIG_LGE_PM_CHARGE_SCENARIO_THERMAL
#define CONFIG_LGE_PM_CHARGE_SCENARIO_FB
#define CONFIG_LGE_PM_CHARGE_SCENARIO_CALL
#define CONFIG_LGE_PM_CHARGE_SAFETY_TIMER
...
static int chgctrl_update_limit(struct chgctrl_limit *limit)
{
	...
	if (value != limit->value) {
		...
		if (limit->active) {
			pr_cc(PR_DEBUG, "%s limit %d by %s\n",
					limit->name, limit->value,
					chgctrl_voter_str[limit->voter]);
		} else {
			pr_cc(PR_DEBUG, "%s limit clear\n", limit->name);
		}
	}
...
static int chgctrl_get_volt_state(struct chgctrl *chgctrl)
{
	...
	/* do not need to check voltage for otp ver.2 */
	if (chgctrl->otp_v2)
