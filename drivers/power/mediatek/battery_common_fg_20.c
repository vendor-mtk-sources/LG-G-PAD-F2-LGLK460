...
static int write_Percentage_Cycle_Count(char *filename, int value);
static int read_Percentage_Cycle_Count(char *filename);
static void check_Percentage_Cycle_Count(unsigned int pre_uisoc2);
static void init_Percentage_Cycle_Count(void);
static int g_percentage_delta=0;
static int g_percentage_Cycle_count = 0;
#endif
/* ////////////////////////////////////////////////////////////////////////////// */
...
//debug
	case POWER_SUPPLY_PROP_CAPACITY:
		BMT_status.UI_SOC2 = val->intval;
		pr_err("set POWER_SUPPLY_PROP_CAPACITY :%d ",BMT_status.UI_SOC2 );
		break;
//debug
#endif
	default:
...
/* CC: modify to have a full power supply status */
	...
	.BAT_HEALTH = POWER_SUPPLY_HEALTH_GOOD,
	.BAT_PRESENT = 1,
#ifdef CONFIG_MTK_EMBEDDED_BATTERY
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (update_cnt == 6) {
		...
		power_supply_changed(bat_psy);
#ifdef CONFIG_LGE_PM_MTK_SAVE_BATTERY_CYCLE_GM_2_0
		check_Percentage_Cycle_Count(pre_uisoc2);
#endif
		pre_uisoc2 = BMT_status.UI_SOC2;
		update_cnt = 0;
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	/* do not show online when input current blocked */
...
void check_battery_exist(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (lge_is_factory_cable_boot())
			battery_log(BAT_LOG_CRTI, "[BATTERY] battery_status = %d (%d) \n", battery_status, i);
#endif
		baton_count += battery_status;

	}

	battery_log(BAT_LOG_CRTI, "[BATTERY] baton_count = %d \n", baton_count);
...
{
#ifdef CONFIG_MACH_MT6750_LV5
	char model_name[] = "LG-M250n";
#endif
...
#ifdef CONFIG_MACH_MT6750_LV5
	if (!strncmp(model_name, lge_get_model_name(), strlen(model_name)))
...
static int battery_probe(struct platform_device *dev)
{
	...
	battery_charging_control(CHARGING_CMD_SET_HV_THRESHOLD, &batt_cust_data.v_charger_max);
#else
...
void init_Percentage_Cycle_Count(void)
{
		if(is_battery_remove_pmic()== 1) // if battery is removed , then the value is initialized
		{
			battery_log(BAT_LOG_CRTI, "g_percentage_Cycle_count initialized by battery_removed ");
			write_Percentage_Cycle_Count("/persist-lg/chargerlogo/percent_cycle",0);
			write_Percentage_Cycle_Count("/persist-lg/chargerlogo/percent_delta",0);
		}

		g_percentage_Cycle_count = read_Percentage_Cycle_Count("/persist-lg/chargerlogo/percent_cycle");
		if (g_percentage_Cycle_count < 0)
		{
			g_percentage_Cycle_count = 0;
			write_Percentage_Cycle_Count("/persist-lg/chargerlogo/percent_cycle",0);
		}

		g_percentage_delta = read_Percentage_Cycle_Count("/persist-lg/chargerlogo/percent_delta");
		if (g_percentage_delta < 0)
		{
			g_percentage_delta = 0;
			write_Percentage_Cycle_Count("/persist-lg/chargerlogo/percent_delta",0);
		}
		battery_log(BAT_LOG_CRTI, "g_percentage_Cycle_count init cycle:%d delta:%d ",g_percentage_Cycle_count,g_percentage_delta);
}

void check_Percentage_Cycle_Count(unsigned int pre_uisoc2)
{
		if(BMT_status.first_soc_est_done == 1)
		{
			g_percentage_delta += abs(pre_uisoc2-BMT_status.UI_SOC2);
			if(g_percentage_delta > 100)
			{
				g_percentage_Cycle_count++;
				write_Percentage_Cycle_Count("/persist-lg/chargerlogo/percent_cycle",g_percentage_Cycle_count);
				battery_log(BAT_LOG_CRTI, "g_percentage_Cycle_count increase :%d",g_percentage_Cycle_count);
				g_percentage_delta -= 100;
			}
			write_Percentage_Cycle_Count("/persist-lg/chargerlogo/percent_delta",g_percentage_delta);
			battery_log(BAT_LOG_CRTI, "g_percentage_delta increase :%d",g_percentage_delta);
...
static int read_Percentage_Cycle_Count(char *filename)
{
	...
	if (fd < 0) {
		pr_err("read_Percentage_Cycle_Count Open Error (%s) (%d) \n",filename, fd);
		goto Error;
	}
	memset(temp_str, 0x00, sizeof(temp_str));
	result = sys_read(fd, temp_str, sizeof(temp_str));
	if (result < 0) {
		pr_err("read_Percentage_Cycle_Count Read Error (%s) (%d)\n",filename, result);
		goto Error;
	}
	result = kstrtoint(temp_str, 10, &value_read);
	if (result != 0) {
		pr_err("read_Percentage_Cycle_Count kstrtoint Error\n");
		goto Error;
	}
...
static int write_Percentage_Cycle_Count(char *filename, int value)
{
	...
	set_fs(KERNEL_DS);
	fd = sys_open(filename, O_WRONLY | O_CREAT | S_IROTH, 0666);
	if (fd < 0) {
		pr_err("write_Percentage_Cycle_Count Open Error (%s) (%d)\n", filename, fd);
		goto Error;
	}

	memset(temp_str, 0x00, sizeof(temp_str));
	sprintf(temp_str, "%d", value);
	result = sys_write(fd, temp_str, sizeof(temp_str));

	if (result < 0) {
		pr_err("write_Percentage_Cycle_Count write Error (%s) (%d) \n",filename, result);
		goto Error;
	}
	value_write = result;
	sys_fsync(fd);
