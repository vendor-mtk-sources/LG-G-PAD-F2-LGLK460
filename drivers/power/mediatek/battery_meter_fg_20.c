...
#ifdef CONFIG_LGE_PM
static const char *battery_name = "TOCAD";
#endif
...
{
	battery_name = (const char*)batt_id;
	bm_err("[JJB] lge.battid = %s\n", battery_name);
...
static struct device_node *battery_id_find_profile(struct device_node *np)
{
	...
	if (of_property_read_string(np, battery_name, &profile_name))
		return NULL;

	node = of_find_node_by_name(np, profile_name);
	bm_info("[JJB] batt_id: %s, node_name: %s\n", battery_name, profile_name);
	if (!node)
		return NULL;
...
int __batt_meter_init_cust_data_from_dt(struct platform_device *dev)
{
...
#ifdef CONFIG_LGE_PM
	BATTERY_ID_REPLACE_NODE(np, "batt_temperature_table_size");
#endif
	if (!of_property_read_u32(np, "batt_temperature_table_size", &val))
		size = (int)val;
	else
...
#endif
	if (!of_property_read_u32(np, "battery_profile_t0_size", &val))
	...
	if (!of_property_read_u32(np, "battery_profile_t1_size", &val))
	...
	if (!of_property_read_u32(np, "battery_profile_t2_size", &val))
	...
	if (!of_property_read_u32(np, "battery_profile_t3_size", &val))
	...
	if (!of_property_read_u32(np, "r_profile_t0_size", &val))
	...
	if (!of_property_read_u32(np, "r_profile_t1_size", &val))
	...
	if (!of_property_read_u32(np, "r_profile_t2_size", &val))
	...
	if (!of_property_read_u32(np, "r_profile_t3_size", &val))
