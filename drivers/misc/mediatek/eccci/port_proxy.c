...
static int ccci_md_log_level = 0x4f4c;
...
{
	struct ccci_modem *ctl;
	/* 30s wait for MD dump finished */
	int retry = 60;

	ctl = ccci_md_get_modem_by_id(md_id);

	if (ccci_md_log_level == MD_LOG_LEVEL_LOW) {
		/* modem reset */
		CCCI_INF_MSG(md_id, CHAR, " Curr log level is low, trigger reset\n");
#if defined(CONFIG_LGE_HANDLE_PANIC)
		if (lge_get_crash_handle_status() &&
			(lge_get_modem_info() != 0)) {
			dump_stack();
...
#endif
		exec_ccci_kern_func_by_md_id(md_id, ID_RESET_MD, NULL, 0);
...
#endif
	} else if ((ccci_md_log_level == MD_LOG_LEVEL_MID) || (ccci_md_log_level == MD_LOG_LEVEL_HIGH)) {
		/* confirm MD dump finished than trigger panic */
		CCCI_INF_MSG(md_id, CHAR, "wait dump done and trigger ram dump....\n");
		if (ctl && ctl->ops && ctl->ops->is_dump_done != NULL) {
			while (retry-- > 0) {
				if (ctl->ops->is_dump_done(ctl)) {
					CCCI_ERROR_LOG(md_id, CHAR,
						"wait dump done and trigger ram dump: get done flag!!!\n");
					break;
				}
				msleep(500);
			}
		} else
			CCCI_ERROR_LOG(md_id, CHAR, "ops->is_dump_done is NULL\n");

		CCCI_INF_MSG(md_id, CHAR, "wait dump done and trigger ram dump: trigger ram dump\n");
		panic("CP Crash");
	} else
		CCCI_INF_MSG(md_id, CHAR, " md log level is invalid, do nothing\n");
}
...
int port_proxy_start_md(struct port_proxy *proxy_p)
{
	...
	((struct ccci_modem *)(proxy_p->md_obj))->ccci_drv_trigger_upload = 0;
...
long port_proxy_user_ioctl(struct port_proxy *proxy_p, int ch, unsigned int cmd, unsigned long arg)
{
	...
	case CCCI_IOC_DRV_ENTER_UPLOAD:
		((struct ccci_modem *)(proxy_p->md_obj))->ccci_drv_trigger_upload = 1;
		CCCI_INF_MSG(md_id, CHAR, "drv trigger upload called by %s\n", current->comm);
		break;

	case CCCI_IOC_LOG_LVL:
		if (copy_from_user(&ccci_md_log_level, (void __user *)arg, sizeof(unsigned int))) {
			CCCI_INF_MSG(md_id, CHAR, "cpy log level fail: copy_from_user fail!\n");
			ret = -EFAULT;
		}
		CCCI_INF_MSG(md_id, CHAR, "cpy log level value:0x%x\n", ccci_md_log_level);
		break;
