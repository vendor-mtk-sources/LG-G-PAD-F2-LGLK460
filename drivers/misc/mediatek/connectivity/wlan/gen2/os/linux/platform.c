...
#define WIFI_SMART_RUNTIME_NVRAM_FILE_NAME   "/mpt/wifi/WIFI"
#define WIFI_SMART_RUNTIME_MTK_CAL_UPDATE   "/data/misc/wifi/mtk_cal_updated"
#else
                    ...
                    IN UINT_32 u4Len, OUT PUINT_16 pu2Data)
{
    if (pu2Data == NULL)
        return FALSE;
    DEBUGFUNC("kalCfgDataRead");
    if (kalIsMTKWifiCalUpdated()) {
        if (nvram_read(WIFI_SMART_RUNTIME_NVRAM_FILE_NAME,
...
BOOLEAN kalCfgDataWrite16(IN P_GLUE_INFO_T prGlueInfo, UINT_32 u4Offset, UINT_16 u2Data)
{
    ...
    DEBUGFUNC("kalCfgDataWrite16");
    if (kalIsMTKWifiCalUpdated()) {
        if (nvram_write(WIFI_SMART_RUNTIME_NVRAM_FILE_NAME,
...
/*----------------------------------------------------------------------------*/
/*!
* \brief API for LG SMART RUNTIME CONFIG
*
* \param[in]
*           prGlueInfo
*           u4Offset
...
*/
/*----------------------------------------------------------------------------*/
BOOLEAN kalIsMTKWifiCalUpdated(void)
{
   ...
   int ret;
   char *tmp_buf;
   unsigned long cal_updated = 0;

   DEBUGFUNC("kalIsMTKWifiCalUpdated");
   ...
   fp = filp_open(WIFI_SMART_RUNTIME_MTK_CAL_UPDATE, O_RDONLY, 0);
   if (IS_ERR(fp)) {
       return FALSE;
   }

   tmp_buf = kmalloc(10, GFP_KERNEL);
   if (!tmp_buf)
      return FALSE;

   ret = kernel_read(fp, 0, tmp_buf, 10);
   filp_close(fp, NULL);
   ...
   if (ret > 0) {
        /* Success to read */
        cal_updated = simple_strtoul(tmp_buf, NULL, 10);
        DBGLOG(REQ, INFO, "cal_updated (%ld)\n", cal_updated);
        if (cal_updated == 1) {
            ret = TRUE;
        } else {
            ret = FALSE;
