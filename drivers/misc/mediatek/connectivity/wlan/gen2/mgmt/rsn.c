...
BOOLEAN rsnSearchAKMSuite(IN P_ADAPTER_T prAdapter, IN UINT_32 u4AkmSuite, OUT PUINT_32 pu4Index)
{
	...
	DBGLOG(RSN, INFO, "u4AkmSuite = %d\n", u4AkmSuite);
	for (i = 0; i < MAX_NUM_SUPPORTED_AKM_SUITES; i++) {
		prEntry = &prAdapter->rMib.dot11RSNAConfigAuthenticationSuitesTable[i];
		DBGLOG(RSN, INFO, "prEntry->dot11RSNAConfigAuthenticationSuite = %d, prEntry->dot11RSNAConfigAuthenticationSuiteEnabled = %d\n",
...
BOOLEAN rsnPerformPolicySelection(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBss)
{
	...
	DBGLOG(RSN, INFO, "prAdapter->rWifiVar.rConnSettings.eAuthMode = %d\n", prAdapter->rWifiVar.rConnSettings.eAuthMode);
	DBGLOG(RSN, INFO, "prBss->fgIEWPA = %d\n", prBss->fgIEWPA);
	DBGLOG(RSN, INFO, "prBss->fgIERSN = %d\n", prBss->fgIERSN);
...
#if CFG_SUPPORT_802_11W
	DBGLOG(RSN, INFO, "prBssRsnInfo->u4AuthKeyMgtSuiteCount = %d\n", prBssRsnInfo->u4AuthKeyMgtSuiteCount);
	if (i != 0)
