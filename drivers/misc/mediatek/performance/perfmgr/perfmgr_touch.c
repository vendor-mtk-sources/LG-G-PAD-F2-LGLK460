...
#define TOUCH_DOWN_TIMEOUT	200
#define TOUCH_UP_TIMEOUT	80
#define FREQ_DECREASE_PERCENTAGE	100	/* keep same freq level */

/*--------------------FUNCTION----------------*/

static int tboost_thread(void *ptr)
{
	int event = 0, core = 0, freq = 0;
	unsigned long flags;
	long ret;
	...
	while (!kthread_should_stop()) {
		...
		while (!atomic_read(&tboost.event)) {
			if (timeout)
				ret = wait_event_timeout(tboost.wq, atomic_read(&tboost.event), timeout);
			else
				wait_event(tboost.wq, atomic_read(&tboost.event));

			if (timeout != 0 && ret == 0) {
				if (event == 1) {
					pr_debug("decrease freq %d KHz\n", freq * FREQ_DECREASE_PERCENTAGE / 100);
#ifdef MTK_BOOST_SUPPORT
					perfmgr_boost(event, core, freq * FREQ_DECREASE_PERCENTAGE / 100);
#endif
				} else if (event == 0) {
					pr_debug("boost end\n");
#ifdef MTK_BOOST_SUPPORT
		...
		if (event == 1) {
			pr_debug("boost start to %d KHz\n", freq);
#ifdef MTK_BOOST_SUPPORT
			perfmgr_boost(event, core, freq);
#endif
			timeout = msecs_to_jiffies(TOUCH_DOWN_TIMEOUT);
		} else {
			pr_debug("touch up\n");
