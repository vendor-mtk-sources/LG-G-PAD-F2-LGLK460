...
typedef struct {
	...
	unsigned int (*dsi_dcs_read_cmdq_lcm_reg_v2_2)(void *cmdq, unsigned char cmd, unsigned char *buffer,
						 unsigned char buffer_size, unsigned char flag);
...
typedef struct {
	...
	unsigned int (*read_by_cmdq_vdo)(void *cmdq, unsigned cmd, unsigned char *buffer,
					unsigned char size, unsigned int flag);
	void (*init)(void);
	void (*suspend)(void);
	void (*resume)(void);
	...
	LCM_setting_table_V3* (*get_init_cmd_addr)(void);
	int (*get_init_cmd_size)(void);
#endif
#if defined(CONFIG_LGE_READER_MODE)
