...
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
		if (ri->batt_present != 1) {
			RTINFO("Battery is not present. Set vfloat to 4.4V\n");
			batvolt_mv = 4400;
		}
#endif
		if (batvolt_mv < 3500)
...
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	if (info->batt_present == 0)
		rt9460_init_regval[3] = 0xda;
#endif
...
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	ri->batt_present = rt9460_batt_pres_status();

	if((ri->batt_present == 1) && (lge_get_board_cable() != LT_CABLE_910K))
		rt9460_chip_disable(ri);
	else
		RTINFO("batt_present = %d, boot_cable = %d => skip chip disable\n", ri->batt_present, lge_get_board_cable());
#else
	rt9460_chip_disable(ri);
#endif
