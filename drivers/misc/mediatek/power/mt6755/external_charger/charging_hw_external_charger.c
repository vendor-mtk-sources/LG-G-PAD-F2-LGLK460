...
static int charging_get_current(void *data)
{
	...
	battery_log(BAT_LOG_CRTI, "[CHARGER] Total Charging Current to %dmA ( Main: %d, Slave: %d)\n",
				cur, main_cur, slave_cur);
...
static int charging_get_ex_charger_det_status(void)
{
	...
	if (rc) {
		battery_log(BAT_LOG_CRTI, "[CHARGER] failed to get VBUS from External charger IC.(%d)\n", rc);
		return status;
	}
