...
void accdet_detect(void)
{
	...
	if (!ret)
		ACCDET_ERROR("[Accdet]accdet_detect:accdet_work return:%d!\n", ret);
}
...
static void multi_key_detection(int current_status)
{
	...
	if (((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) != IRQ_STATUS_BIT) || eint_accdet_sync_flag) {
		...
		ACCDET_ERROR("[Accdet]plug out side effect key press, do not report key = %d\n", cur_key);
		cur_key = NO_KEY;
	}
	if (current_status)
		cur_key = NO_KEY;
...
#define PIN_ADC_CHANNEL 5
	...
	ACCDET_DEBUG("[Accdet]check_cable_type:[%s]current AB = %d\n",
...
#elif defined(CONFIG_LGE_ACCDET)
	if (1 == eint_accdet_sync_flag) {
		ACCDET_ERROR( " [Accdet] accdet_work_callback cable_type = %d\n",cable_type);
		if (cable_type == NO_DEVICE) {
			input_report_switch(kpd_accdet_dev, SW_HEADPHONE_INSERT, 0);
			input_report_switch(kpd_accdet_dev, SW_MICROPHONE_INSERT, 0);
			input_sync(kpd_accdet_dev);
...
void accdet_pmic_Read_Efuse_HPOffset(void)
{
	...
	ACCDET_DEBUG("[accdet]efusevalue = 0x%x, auxadc_offset = %d\n", efusevalue, accdet_auxadc_offset);
}
