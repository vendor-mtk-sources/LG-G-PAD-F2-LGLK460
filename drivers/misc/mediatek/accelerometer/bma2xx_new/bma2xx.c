...
static int bma2xx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		printk("zhou invalid direction: %d\n", obj->hw->direction);
		goto exit;
	}
	...
	for(retry = 0; retry < 3; retry++){
		...
		{
			printk("zhou bma2xx_device init cilent fail time: %d\n", retry);
			continue;
		}
	}
	if(err != 0)
	...
	if(err)
	{
		printk("zhou register acc control path err\n");
		goto exit_kfree;
	}
	...
	if(err)
	{
		printk("zhou register acc data path err\n");
		goto exit_kfree;
	}
	...
	if(err)
	{
		printk("zhou register gsensor batch support err = %d\n", err);
		goto exit_create_attr_failed;
	}
