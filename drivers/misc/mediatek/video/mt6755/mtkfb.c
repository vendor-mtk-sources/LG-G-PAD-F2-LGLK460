	...
	case COMPAT_MTKFB_SET_VIDEO_LAYERS:
	{
		...
		if (copy_from_user(compat_layerInfo, (void __user *)arg, compat_layerInfo_size)) {
			...
			for (i = 0; i < VIDEO_LAYER_COUNT; ++i) {
				...
				if (layerInfo.layer_id >= TOTAL_OVL_LAYER_NUM) {
					DISPMSG("COMPAT_MTKFB_SET_VIDEO_LAYERS, layer_id invalid=%d\n",
						     layerInfo.layer_id);
					continue;
				}
				input =
...
#if defined(CONFIG_LGE_VSYNC_SKIP) || defined(CONFIG_LGE_READER_MODE)
...
{
#if defined(CONFIG_LGE_READER_MODE) || defined(CONFIG_LGE_VSYNC_SKIP)
    int rc;
