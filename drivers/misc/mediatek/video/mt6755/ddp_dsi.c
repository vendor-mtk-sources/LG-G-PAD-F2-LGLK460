...
#endif
				ret = 1;
				DISPDBG("[ESD]CMP index %d return value 0x%x,para_list[%d]=0x%x\n", index,
...
uint32_t DSI_dcs_read_lcm_reg_v2_2(DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq,
				   uint8_t cmd, uint8_t *buffer, uint8_t buffer_size, uint8_t flag)
{
	int dsi_i = 0;
	DSI_T0_INS t0;
	DSI_T0_INS t1;
	...
	if (flag == 0) {
		...
		DSI_OUTREGBIT(cmdq, DSI_INT_ENABLE_REG, DSI_REG[dsi_i]->DSI_INTEN,
			      RD_RDY, 1);
		DSI_OUTREGBIT(cmdq, DSI_INT_ENABLE_REG, DSI_REG[dsi_i]->DSI_INTEN,
			      CMD_DONE, 1);
		...
		if (hSlot) {
			...
			pr_err("DSI read save RX data fail\n");
		}
		/* 3. write RX_RACK */
		DSI_OUTREGBIT(cmdq, DSI_RACK_REG, DSI_REG[dsi_i]->DSI_RACK,
			      DSI_RACK, 1);
		...
		if (hSlot) {
			...
			pr_err("DSI read hSlot is empty\n");
		}

		{
			unsigned int i;

			DISPMSG("DSI read begin  --------------------\n");
			DISPMSG("DSI_RX_STA : 0x%08x\n",
				AS_UINT32(&DSI_REG[0]->DSI_TRIG_STA));
			DISPMSG("DSI_CMDQ_SIZE	: %d\n",
				  AS_UINT32(&DSI_REG[0]->DSI_CMDQ_SIZE));
			for (i = 0; i < DSI_REG[0]->DSI_CMDQ_SIZE.CMDQ_SIZE; i++) {
				DISPMSG("DSI_CMDQ_DATA%d : 0x%08x\n", i,
					  AS_UINT32(&DSI_CMDQ_REG[0]->data[i]));
			}
			DISPMSG("DSI read end ----------------------------\n");
			DISPMSG("CMDQ read data start ----------------\n");
			DISPMSG("CMDQ read_data0	: 0x%02x%02x%02x%02x\n",
				  read_data0.byte3, read_data0.byte2, read_data0.byte1, read_data0.byte0);
			DISPMSG("CMDQ read_data1	: 0x%02x%02x%02x%02x\n",
				  read_data1.byte3, read_data1.byte2, read_data1.byte1, read_data1.byte0);
			DISPMSG("CMDQ read_data2	: 0x%02x%02x%02x%02x\n",
				  read_data2.byte3, read_data2.byte2, read_data2.byte1, read_data2.byte0);
			DISPMSG("CMDQ read_data3	: 0x%02x%02x%02x%02x\n",
				  read_data3.byte3, read_data3.byte2, read_data3.byte1, read_data3.byte0);
			DISPMSG("CMDQ read data end -------------------\n");
...
unsigned int DSI_dcs_read_cmdq_lcm_reg_v2_2_wrapper_DSI0(void *cmdq, uint8_t cmd,
				uint8_t *buffer, uint8_t buffer_size, uint8_t flag)
{
	return DSI_dcs_read_lcm_reg_v2_2(DISP_MODULE_DSI0, cmdq, cmd, buffer, buffer_size, flag);
}
...
int ddp_dsi_set_lcm_utils(DISP_MODULE_ENUM module, LCM_DRIVER *lcm_drv)
{
	...
	if (module == DISP_MODULE_DSI0) {
		...
		utils->dsi_dcs_read_cmdq_lcm_reg_v2_2 = DSI_dcs_read_cmdq_lcm_reg_v2_2_wrapper_DSI0;
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			...
			if (ret) {
#if defined(CONFIG_LGE_LCM_KD_HX8394A_TH8)
				printk("[DSI]enter cmp ESD CHECK FAILED!\n");
				printk("[DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x\n",
									read_data[0].byte0, read_data[0].byte1);
				...
				printk("[DSI]enter cmp check_table cmd=0x%x, count=0x%x,\
									list[0]=0x%x, list[1]=0x%x, list[2]=0x%x, list[3]=0x%x, list[4]=0x%x,\
									list[5]=0x%x, list[6]=0x%x, list[7]=0x%x, list[8]=0x%x, list[9]=0x%x\n",
									...
									dsi_params->lcm_esd_check_table[i].para_list[4],
									dsi_params->lcm_esd_check_table[i].para_list[5],
									dsi_params->lcm_esd_check_table[i].para_list[6],
									dsi_params->lcm_esd_check_table[i].para_list[7],
									dsi_params->lcm_esd_check_table[i].para_list[8],
									dsi_params->lcm_esd_check_table[i].para_list[9]);
				return ret;
#else
				printk("enter cmp ESD CHECK FAILED!\n");
				printk("[DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x\n", read_data[0].byte0, read_data[0].byte1);
				printk("[DSI]enter cmp read_data1 byte0=0x%x, byte1=0x%x, byte2=0x%x\n", read_data[1].byte0, read_data[1].byte1, read_data[1].byte2);
				...
				printk("enter cmp ESD CHECK SUCCESS!\n");
			}
#endif
		}
#if defined(CONFIG_LGE_LCM_KD_HX8394A_TH8)
		if (!ret) {
			printk("[DSI]enter cmp ESD CHECK SUCCESS!\n");
		}
#endif
