...
#ifdef CONFIG_TOVIS_INCELL_TD4100_HD_LV7_SPR
...
int _dsi_read_cmdq_vdo(disp_lcm_handle *plcm, unsigned cmd, unsigned char *buffer,
	unsigned char size)
{
	int ret = 0;
	...
	if (primary_display_is_video_mode() != 0) {
		...
		DISPMSG("cmd %x buffer %p size %d\n", cmd, buffer, size);
		/* 3. write instruction(read from lcm) */
		DISPMSG("VDO=>disp_lcm_util_set_read_cmdq_vdo_v2\n");
		disp_lcm_util_set_read_cmdq_vdo_v2(plcm, cmdq_handle, cmd, buffer, size, 0);
	...
	primary_display_manual_lock();
	/* 7. check data */
	disp_lcm_util_set_read_cmdq_vdo_v2(plcm, cmdq_handle, cmd, buffer, size, 1);
	DISPMSG("VDO<=disp_lcm_util_set_read_cmdq_vdo_v2\n");
	primary_display_manual_unlock();
	/* 8. destroy */
	cmdqRecDestroy(cmdq_handle);
...
int primary_display_dsi_read_cmdq_vdo(unsigned cmd, unsigned char *buffer, unsigned char size)
{
	int ret = 0;
	...
	primary_display_manual_lock();
	if (pgc->state == DISP_SLEPT) {
		DISPERR("Sleep State read DSI invald\n");
		primary_display_manual_unlock();
	} else {
		primary_display_manual_unlock();
		if (primary_display_cmdq_enabled()) {
			DISPMSG("_dsi_read_cmdq_vdo+  cmd %x buffer %p size %d\n", cmd, buffer, size);
			_dsi_read_cmdq_vdo(pgc->plcm, cmd, buffer, size);
			DISPMSG("_dsi_read_cmdq_vdo-\n");
		}
		atomic_set(&delayed_trigger_kick, 1);
	}
	DISPMSG("%s-\n", __func__);
