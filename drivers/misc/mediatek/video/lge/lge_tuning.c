...
static ssize_t mtkfb_lge_init_cmd_find_show(struct kobject *kobj, struct
	...
	LCM_setting_table_V3 * init_cmd_str = NULL;
	...
	if (lge_pgc->plcm){
		...
		init_cmd_str = lcm_drv->get_init_cmd_addr();
		str_size = lcm_drv->get_init_cmd_size();
	}

	if(lcm_drv == NULL || init_cmd_str == NULL || str_size == 0)
		return -EINVAL;
...
static ssize_t mtkfb_lge_init_cmd_edit_store(struct kobject *kobj, struct
	...
	if( !count || reg_num_idx == -1 || lcm_drv == NULL || init_cmd_str == NULL)
		return -EINVAL;
