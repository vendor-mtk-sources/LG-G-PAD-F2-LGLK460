...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI556_SENSOR_ID_QH_SUB,	//0x0559
...
#ifdef QH_SUB_OTP_CALI
struct hi556_otp_struct qh_sub_otp; /*LGE_CHANGE, 2017-04-24, enabled hi556 otp, soojong.jin@lge.com*/
...
static kal_uint32 hi556_qh_sub_otp_read_info(void)
{
	kal_uint32	i = 0, size = 0, checksum = 0;
	kal_uint8	*data = NULL;

	/* Module info */
	qh_sub_otp.infoStr.info_flag = hi556_otp_read_byte(0x0EAC);
	size = sizeof(struct hi556_info_struct) / sizeof(kal_uint8) - 1;//Except flag reg
	LOG_DEBUG("info_flag = 0x%x, info_size = %d.\n", qh_sub_otp.infoStr.info_flag, size);
	if (qh_sub_otp.infoStr.info_flag == 0x01) {
		hi556_otp_read_block(0x0EAD, size, &qh_sub_otp.infoStr.vendor_id);
	} else if (qh_sub_otp.infoStr.info_flag == 0x13) {
		hi556_otp_read_block(0x0EBB, size, &qh_sub_otp.infoStr.vendor_id);
	} else if (qh_sub_otp.infoStr.info_flag == 0x37) {
		hi556_otp_read_block(0x0EC9, size, &qh_sub_otp.infoStr.vendor_id);
	} else {
		LOG_DEBUG("Invalid module info.\n");
		qh_sub_otp.infoStr.info_flag = 0;
	}

	data = &qh_sub_otp.infoStr.vendor_id;
	for (i = 0; i < size - 2; i++)//Except checksum reg
		checksum += data[i];
	checksum = checksum % 0xFFFF + 1;

	if ((checksum & 0xFFFF) != (((qh_sub_otp.infoStr.info_checksum_h << 8) \
					| (qh_sub_otp.infoStr.info_checksum_l & 0xFF)) & 0xFFFF)) {
...
static kal_uint32 hi556_qh_sub_otp_read_wb(void)
{
	kal_uint32	i = 0, size = 0, checksum = 0;
	kal_uint8	*data = NULL;

	/* AWB 5100k */
	qh_sub_otp.wb5100k.wb_flag = hi556_otp_read_byte(0x0401);//Except flag reg
	size = sizeof(struct hi556_wb5100k) / sizeof(kal_uint8) - 1;
	LOG_INF("wb5100k_flag = 0x%x, wb5100k_size = %d.\n", qh_sub_otp.wb5100k.wb_flag, size);
	if (qh_sub_otp.wb5100k.wb_flag == 0x01) {
		hi556_otp_read_block(0x0402, size, &qh_sub_otp.wb5100k.RGr_ratio_l);
	} else if (qh_sub_otp.wb5100k.wb_flag == 0x13) {
		hi556_otp_read_block(0x040A, size, &qh_sub_otp.wb5100k.RGr_ratio_l);
	} else if (qh_sub_otp.wb5100k.wb_flag == 0x37) {
		hi556_otp_read_block(0x0412, size, &qh_sub_otp.wb5100k.RGr_ratio_l);
	} else {
		LOG_DEBUG("Invalid AWB(5100k) data.\n");
		qh_sub_otp.wb5100k.wb_flag = 0;
	}

	data = &qh_sub_otp.wb5100k.RGr_ratio_l;
	for (i = 0; i < size - 2; i++)//Except checksum reg
		checksum += data[i];
	checksum = checksum % 0xFFFF + 1;
 	LOG_DEBUG("qh_sub 5100k checksum = %x, readchecksum = %x\n", (checksum & 0xFFFF), (((qh_sub_otp.wb5100k.wb_checksum_h << 8) | (qh_sub_otp.wb5100k.wb_checksum_l & 0xFF)) & 0xFFFF));
	if ((checksum & 0xFFFF) != (((qh_sub_otp.wb5100k.wb_checksum_h << 8) \
					| (qh_sub_otp.wb5100k.wb_checksum_l & 0xFF)) & 0xFFFF)) {
		LOG_DEBUG("Checksum error, Invalid AWB(5100k).\n");
		qh_sub_otp.wb5100k.wb_flag = 0;
	}

	/* AWB 3000k */
	qh_sub_otp.wb3000k.wb_flag = hi556_otp_read_byte(0x041A);
	size = sizeof(struct hi556_wb3000k) / sizeof(kal_uint8) - 1;//Except flag reg
	LOG_INF("wb3000k_flag = 0x%x, wb3000k_size = %d.\n", qh_sub_otp.wb3000k.wb_flag, size);
	if (qh_sub_otp.wb3000k.wb_flag == 0x01) {
		hi556_otp_read_block(0x041B, size, &qh_sub_otp.wb3000k.RGr_ratio_l);
	} else if (qh_sub_otp.wb3000k.wb_flag == 0x13) {
		hi556_otp_read_block(0x0423, size, &qh_sub_otp.wb3000k.RGr_ratio_l);
	} else if (qh_sub_otp.wb3000k.wb_flag == 0x37) {
		hi556_otp_read_block(0x042B, size, &qh_sub_otp.wb3000k.RGr_ratio_l);
	} else {
		LOG_DEBUG("Invalid AWB(3000k) data.\n");
		qh_sub_otp.wb3000k.wb_flag = 0;
	}

	checksum = 0;
	data = &qh_sub_otp.wb3000k.RGr_ratio_l;
	for (i = 0; i < size - 2; i++)//Except checksum reg
		checksum += data[i];
	checksum = checksum % 0xFFFF + 1;

    LOG_DEBUG("qh_sub 3000k checksum = %x, readchecksum = %x\n", (checksum & 0xFFFF), (((qh_sub_otp.wb3000k.wb_checksum_h << 8) | (qh_sub_otp.wb3000k.wb_checksum_l & 0xFF)) & 0xFFFF));
	if ((checksum & 0xFFFF) != (((qh_sub_otp.wb3000k.wb_checksum_h << 8) \
					| (qh_sub_otp.wb3000k.wb_checksum_l & 0xFF)) & 0xFFFF)) {
...
static kal_uint32 hi556_qh_sub_otp_read_lsc(void)
{
	kal_uint32	i = 0, size = 0, checksum = 0;
	kal_uint8	*data = NULL;

	/* LSC */
	qh_sub_otp.lscStr.lsc_flag = hi556_otp_read_byte(0x0433);
	size = sizeof(struct hi556_lsc_struct) / sizeof(kal_uint8) - 1;//Except flag reg
	LOG_INF("lsc_flag = 0x%x, lsc_size = %d.\n", qh_sub_otp.wb3000k.wb_flag, size);
	if (qh_sub_otp.lscStr.lsc_flag == 0x01) {
		hi556_otp_read_block(0x0434, size, &qh_sub_otp.lscStr.lsc_data[0]);
	} else if (qh_sub_otp.lscStr.lsc_flag == 0x13) {
		hi556_otp_read_block(0x07AA, size, &qh_sub_otp.lscStr.lsc_data[0]);
	} else if (qh_sub_otp.lscStr.lsc_flag == 0x37) {
		hi556_otp_read_block(0x0B20, size, &qh_sub_otp.lscStr.lsc_data[0]);
	} else {
		LOG_DEBUG("Invalid LSC data.\n");
		qh_sub_otp.lscStr.lsc_flag = 0;
	}

	data = &qh_sub_otp.lscStr.lsc_data[0];
	for (i = 0; i < size - 2; i++)//Except checksum reg
		checksum += data[i];
	checksum = checksum % 0xFFFF + 1;

        LOG_DEBUG("qh_sub lsc checksum = %x, readchecksum = %x\n", (checksum & 0xFFFF), (((qh_sub_otp.lscStr.lsc_checksum_h << 8) | (qh_sub_otp.lscStr.lsc_checksum_l & 0xFF)) & 0xFFFF));
	if ((checksum & 0xFFFF) != (((qh_sub_otp.lscStr.lsc_checksum_h << 8) \
					| (qh_sub_otp.lscStr.lsc_checksum_l & 0xFF)) & 0xFFFF)) {
...
#if 0 
static kal_uint32 hi556_qh_sub_wb_apply(void)
{
	...
	RG_ratio_unit = (qh_sub_otp.wb5100k.RGr_ratio_h << 8) | (qh_sub_otp.wb5100k.RGr_ratio_l & 0xFF);
	BG_ratio_unit = (qh_sub_otp.wb5100k.BGr_ratio_h << 8) | (qh_sub_otp.wb5100k.BGr_ratio_l & 0xFF);
	...
	return qh_sub_otp.wb5100k.wb_flag;
}
#endif
/*LGE_CHANGE_S, 2017-04-20, blocked sensor awb calibration, soojong.jin@lge.com*/
#endif
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = return_sensor_id() + 3;	//For QH SUB id is 0x0559
			if (*sensor_id == imgsensor_info.sensor_id) {
#ifdef QH_SUB_OTP_CALI
				if(qh_sub_otp.isLoaded == KAL_FALSE){  /*LGE_CHANGE, 2017-05-15, otp reading only one time, soojong.jin@lge.com*/
					sensor_init();
					hi556_otp_setting();
					hi556_qh_sub_otp_read_info();
					if ((qh_sub_otp.infoStr.info_flag != 0) && (qh_sub_otp.infoStr.vendor_id == 0x53) \
							&& (qh_sub_otp.infoStr.module_code == 0xFF)) {
						LOG_DEBUG("i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
						hi556_qh_sub_otp_read_wb();
						hi556_qh_sub_otp_read_lsc();
						hi556_otp_mode_end();
						qh_sub_otp.isLoaded = KAL_TRUE;
						return ERROR_NONE;
					}
					hi556_otp_mode_end();
				}
				else {
					LOG_DEBUG("[LGE][OTP] qh_sub_otp is already loaded");
				}
#else
				LOG_DEBUG("i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
...
/*LGE_CHANGE_S, 2017-04-20, blocked sensor awb calibration, soojong.jin@lge.com*/
#if 0//def QH_SUB_OTP_CALI
	if (qh_sub_otp.wb5100k.wb_flag != 0)
		hi556_qh_sub_wb_apply();
#endif
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
       ...
       //[LGE_UPDATE_S][kipo.yun@lge.com][2015-11-23] update FOV value
        case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
        {
            ...
            *pHorFOV = (0x45)<<16 | (0x45); // 16:9 = 69, 4:3 = 69 // (69)<<16 | (60) //update FOV value Hi-553 69
