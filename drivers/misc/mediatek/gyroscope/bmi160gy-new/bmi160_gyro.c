...
static int bmg_ReadCalibration_file(struct i2c_client *client)
{
	...
	for (i = 0; i < 6; i++) {
		...
		ret = kstrtoint(temp_str, 10, &cal_read[i]);
	}
	...
	GYRO_LOG("data : %d %d %d / %d %d %d\n",
		 ...
		 obj->cali_sw[BMG_AXIS_OLD_X], obj->cali_sw[BMG_AXIS_OLD_Y],
		 obj->cali_sw[BMG_AXIS_OLD_Z]);
...
static int bmg_do_calibration(struct i2c_client *client)
{
	...
	do {
		...
		if ((abs(bmi160_udata.x - bmi160_udata_pre.x) >
		     BMI160_GYRO_SHAKING_DETECT_THRESHOLD)
		    || (abs(bmi160_udata.y - bmi160_udata_pre.y) >
			BMI160_GYRO_SHAKING_DETECT_THRESHOLD)
		    || (abs(bmi160_udata.z - bmi160_udata_pre.z) >
			BMI160_GYRO_SHAKING_DETECT_THRESHOLD)) {
			shake_count++;
		} else {
...
static ssize_t store_run_fast_calibration(struct device_driver *ddri, const char *buf, size_t count)
{
	int res = 0;
	struct bmg_i2c_data *obj = obj_i2c_data;
...
*/
	...
	if (gyro_self_test_start < 2) {
		comres +=
		    bmg_i2c_read_block(obj->client, BMI160_USER_GYRO_SELF_TEST_START__REG,
				       &v_data_u8r, 1);
		if (comres == 0) {
			v_data_u8r =
			    BMG_SET_BITSLICE(v_data_u8r, BMI160_USER_GYRO_SELF_TEST_START,
					     gyro_self_test_start);
			GYRO_ERR("bmi160 start setting %x\n", v_data_u8r);
			comres +=
			    bmg_i2c_write_block(obj->client, BMI160_USER_GYRO_SELF_TEST_START__REG,
						&v_data_u8r, 1);
		}
	} else {
...
static int bmi160_get_gyro_self_test(u8 *gyro_self_test_ok)
{
	...
	comres +=
	    bmg_i2c_read_block(obj->client, BMI160_USER_STATUS_GYRO_SELFTEST_OK__REG, &v_data_u8r,
			       1);
