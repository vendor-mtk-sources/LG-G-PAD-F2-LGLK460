...
static void Topck_Enable(bool enable)
{
	...
	pr_warn("Topck_Enable(3)\n");
	mutex_unlock(&Ana_Clk_Mutex);
	pr_warn("Topck_Enable(4)\n");
}
...
static const int64 dBFactor_Nom[MAX_HP_GAIN_LEVEL] = {
	13045, /* -8dB */
	14637, /* -7dB */
	16423, /* -6dB */
	18427, /* -5dB */
...
static const int64 dBFactor_Den = 32768;
...
void CalculateDCCompenForEachdB(void)
{
	...
	for (i = 0; i < MAX_HP_GAIN_LEVEL; i++) {  /* 0:8dB, 1:7dB, ...,  18:-10dB */
		mHplOffset_CompenBydB[i] = DIV_ROUND_CLOSEST((int64)(mHplTrimOffset - TrimOffset) * dBFactor_Nom[i],
			dBFactor_Den);
		mHprOffset_CompenBydB[i] = DIV_ROUND_CLOSEST((int64)(mHprTrimOffset - TrimOffset) * dBFactor_Nom[i],
			dBFactor_Den);
...
void CalculateSPKHP_DCCompenForEachdB(void)
{
	...
	for (i = 0; i < MAX_HP_GAIN_LEVEL; i++) {  /* 0:8dB, 1:7dB, ...,  18:-10dB */
		mHplSpkOffset_CompenBydB[i] = DIV_ROUND_CLOSEST((int64)(mHplSpkTrimOffset[i] - TrimOffset) *
			dBFactor_Nom[i], dBFactor_Den);
		mHprSpkOffset_CompenBydB[i] = DIV_ROUND_CLOSEST((int64)(mHprSpkTrimOffset - TrimOffset) *
			dBFactor_Nom[i], dBFactor_Den);
...
#define DCOFFSET_RAMP_STEP_HPL 3
	...
	DCoffsetValue = DIV_ROUND_CLOSEST(OffsetTrimming * 10589, 4096);
...
static void TurnOnDacPower(void)
{
	...
	ClsqEnable(true);
	Topck_Enable(true);
	pr_warn("TurnOnDacPower(1)\n");
	udelay(250);
	pr_warn("TurnOnDacPower(2)\n");
	...
	if (enable) {
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				...
				/* ALPS03263596 avoid pop up noise when it insert headset. */
...
static void mtk_mt6331_codec_dev_shutdown(struct platform_device *pdev)
{
	pr_warn("%s:\n", __func__);
#ifdef CONFIG_MACH_MT6750_TH8
	Ext_Speaker_Amp_Change(false);
...
static struct platform_driver mtk_codec_6331_driver = {
	...
	.shutdown = mtk_mt6331_codec_dev_shutdown,
};
